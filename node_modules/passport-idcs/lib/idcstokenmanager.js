...
/**
 * get tenant specifi cache key based on jti and tenant name
 * @param tokenDecoded
 * @param tenant
 * @returns {string}
 */
function getTokenCacheKeyForTenant(tokenDecoded, tenant){
    logger.trace("getTokenCacheKeyForTenant, tokenDecoded: "+ JSON.stringify(tokenDecoded) +", tenant: "+ tenant);
    if(tokenDecoded){
        var jti = tokenDecoded[IDCSConstants.TOKEN_CLAIM_JTI];
        return tenant+":"+jti;
    }else{
        logger.error("getTokenCacheKeyForTenant, error : decoded token is empty");
    }
}
...
IdcsTokenManager.prototype.verifyToken = function (token) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        var ttl = getTTLfromClaim(tokenDecoded.payload);
        var key;
        if(ttl > 0 && tokenCache){
            key = getTokenCacheKeyForTenant(tokenDecoded.payload,options[IDCSConstants.CLIENT_TENANT]);
            if(key){
